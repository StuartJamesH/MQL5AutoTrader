...
MIN_DELTA = 0.001    # minimum val-loss improvement required to reset early-stop counter
BASE_LR = 1e-4
WEIGHT_DECAY = 8e-4  # increased: noisier relaxed labels need stronger L2
...
# TCN: kernel_size=3, num_layers=6 → TCN receptive field = 253 bars (via 2-conv-per-block formula).
# Attention pooling aggregates all SEQ_LEN=256 positions, so the full 256-bar context is used.
tcn_model_params = {
    "input_dim": None,
    "hidden_channels": 256,
    "num_layers": 6,
    "kernel_size": 3,
    "num_classes": 3,
    "dropout": 0.35,        # increased: noisier relaxed labels need stronger dropout
    "dropout_out": 0.50,   # increased to match
    "attn_heads": 4,
    "attn_dropout": 0.08,
...
loss_params_template = {
    'alpha':             None,
    'gamma':             2.0,    # reduced: less focal weight on noisy hard examples
    'trade_classes':     (0, 2),
    'pr_weight':         8.0,    # reduced: pr/recall tension was causing epoch-7 recall collapse
    'recall_floor':      0.30,   # raised: earlier hinge activation prevents recall collapse
    'rec_floor_weight':  30.0,   # quadratic hinge strength
...
def main() -> None:
    ...
    weights = np.power(raw_weights, 0.5)
    ...
    try:
        for epoch in range(NUM_EPOCHS):
            ...
            if eval_pack["val_loss"] < history["best_val_loss"] - MIN_DELTA:
