...
#   model_params       — LSTMAttentionSEClassifier kwargs (input_dim & bias_init injected automatically)
# ══════════════════════════════════════════════════════════════════════════════

SWEEP_CONFIGS = [
    # ── v1 — Baseline: 2 layers, 128 hidden, bidirectional ──────────────────
    {
        'version_tag':        'v1',
    ...
    # ── v2 — Deeper: 3 layers ────────────────────────────────────────────────
    {
        ...
        'model_params':         {
            ...
            'attn_heads':         8,
            'attn_dropout':       0.05,
            'se_context_window':  64,  # Increased from 32
            'use_learned_query':  True,
        },
    },
    # ── v3 — Higher bias offset (precision-focused) ─────────────────────────
    {
        'version_tag':        'v3',
        ...
        'model_params':         {
            ...
            'num_classes':        3,
            'bidirectional':      True,
            'dropout':            0.10,  # Reduced from 0.15
            'dropout_out':        0.35,  # Reduced from 0.50
            'attn_heads':         8,
            'attn_dropout':       0.05,
            'se_context_window':  32,
            'use_learned_query':  True,
...
if __name__ == '__main__':
    ...
    sweep_path = os.path.join('..', 'Engine', 'Model Packs',
                              f'{ds_title}_LSTM_sweep_{today}_comparison.json')
