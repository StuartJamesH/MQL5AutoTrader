"""
LSTM Loss Params Sweep Trainer
==============================
Trains multiple LSTMAttentionSEClassifier configurations sequentially on a single dataset,
varying loss function parameters across runs while using a fixed model architecture.
Saves a model pack to Engine/Model Packs/ after each run.

Usage (from ModelWorkbench/ directory):
    python train_sweep_loss.py

To add or remove configs, edit SWEEP_CONFIGS below.
All other shared parameters (dataset, labels, model architecture, optimiser) are in the CONFIG section.
"""
...
# ── Base loss params (shared default — overrides applied per sweep config) ────
LOSS_PARAMS_BASE = {
...
# ── Fixed model architecture (shared across all sweep runs) ──────────────────
MODEL_PARAMS = {
    'hidden_dim':        256,
    'num_layers':        3,
    'num_classes':       3,
...
#   loss_params_overrides — dict of LOSS_PARAMS_BASE keys to override (sparse diff only)
#                           Keys not listed inherit from LOSS_PARAMS_BASE.
#                           alpha is always injected at runtime; do not include it here.
# ══════════════════════════════════════════════════════════════════════════════

SWEEP_CONFIGS = [
    # ── v1 — Baseline: LOSS_PARAMS_BASE unchanged ────────────────────────────
    {
        'version_tag':           'v1',
        'trade_bias_offset':     0.10,
        'loss_params_overrides': {},
    },
    # ── v2 — Higher focal gamma + stronger pr_weight ─────────────────────────
    {
        'version_tag':           'v2',
        'trade_bias_offset':     0.10,
        'loss_params_overrides': {
            'gamma':    3.5,    # Increased from 2.5 — steeper focus on hard examples
            'pr_weight': 20.0,  # Increased from 15.0 — stronger precision emphasis
        },
    },
    # ── v3 — Higher recall floor + stronger floor weight + direction penalty ──
    {
        'version_tag':           'v3',
        'trade_bias_offset':     0.10,
        'loss_params_overrides': {
            'recall_floor':      0.35,   # Increased from 0.25 — tighter recall floor
            'rec_floor_weight':  120.0,  # Increased from 80.0 — harder penalty below floor
            'direction_penalty': 2.0,    # Increased from 1.5 — heavier wrong-direction cost
        },
    },
]
...
def train_one_config(cfg, data, device):
    ...
    loss_params_overrides = cfg.get('loss_params_overrides', {})

    # Effective loss params for this run
    run_loss_params = {**LOSS_PARAMS_BASE, **loss_params_overrides}

    print(f'\n{"=" * 70}')
    print(f'TRAINING CONFIG: {version_tag}')
    print(f'  loss_params_overrides: {loss_params_overrides}')
    print(f'  effective_loss_params: {run_loss_params}')
    ...
    MODEL_TYPE = 'LSTM_LossSweep'
    print(f'Model params: {sum(p.numel() for p in model.parameters()):,}')

    # Loss & optimiser
    class_weights = class_weights_cpu.clone().to(device)
    loss_params = {**run_loss_params, 'alpha': class_weights}
    ...
    summary = {
        ...
        'loss_params_overrides': {k: (list(v) if isinstance(v, tuple) else v)
                                  for k, v in loss_params_overrides.items()},
        'loss_params':          {k: (list(v) if isinstance(v, tuple) else v)
                                  for k, v in run_loss_params.items()},
        'bias_init':            bias_init,
        'class_weights':        weights_norm.tolist(),
...
if __name__ == '__main__':
    ...
    sweep_comparison = {
        ...
        'shared_config': {
            ...
            'label_params':     {k: (list(v) if isinstance(v, tuple) else v)
                                  for k, v in label_params.items()},
            'loss_params_base': {k: (list(v) if isinstance(v, tuple) else v)
                                  for k, v in LOSS_PARAMS_BASE.items()},
        ...
        'configs': [
            {
                ...
                'loss_params_overrides': {k: (list(v) if isinstance(v, tuple) else v)
                                          for k, v in c.get('loss_params_overrides', {}).items()},
            }
    ...
    sweep_path = os.path.join('..', 'Engine', 'Model Packs',
                              f'{ds_title}_LSTM_LossSweep_{today}_comparison.json')
