## Run command python Engine\\run.py --source mt5 --mt5-mode live --symbol EURUSD.a
import pandas as pd
import pickle
...
from Learn.features import add_all_features
...
if __name__ == "__main__":
    ...
    with open('Engine/Model Packs/EURUSD_1minute_LSTM_TripleBarrier_HiLow_256seq_20251229__DEV_model.pkl', 'rb') as f:
    ...
    if model_info['model_type'] in ['LSTM_TripleBarrier', 'LSTM_TripleBarrier_HiLow']:
        from Learn.Models import LSTMClassifier, LSTMAttentionSEClassifier
    ...
    parser.add_argument('--symbol', default='EURUSD', help='Symbol to use for MT5 mode')
    ...
    strategy = TripleBarrierHiLow(args.symbol, model=model, model_pack=model_pack, risk=20, patience=1, mt5_executor=executor, data_handler=data, maxpos=1.5)
